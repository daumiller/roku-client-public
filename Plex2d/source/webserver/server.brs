...
function server_init(params as Dynamic)
    ...
    if sockin=invalid then stop

    ' reuse address
    if not sockin.setReuseAddr(true) then stop

    ' bind socket
    addrin = CreateObject("roSocketAddress")
    addrin.setPort(m.port)
    if not sockin.setAddress(addrin) then stop

    ' listen on socket
    if not sockin.listen(m.max_connections) then stop
    ...
    Info("Server listening on "+addrin.getAddress())
end function
...
function server_close()
    ...
    ' final stats
    Info("Server Stats: " + UnixNL() + m.stats())
end function
