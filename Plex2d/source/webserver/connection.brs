...
function AcceptConnection(server as Object)
    ...
    if not sockin.eOK() then Error("Connection accept() - " + tostr(sockin.status()))
...
function connection_accepted(server as Object, socket as Object)
    ...
    Info("Accepted connection @" + m.client)
...
function connection_check_result(byteCount as Integer, op as String) as Boolean
    ...
    if byteCount>0
        ...
        'Info(op + " would have blocked @"+m.client)
    else
        if m.socket.eSuccess()
            Info(op + " peer closed @"+m.client)
        else
            Error(op + " transfer error @"+m.client)
        end if
        m.close = true
        m.setState(m.DONE)
    end if
...
function connection_poll_receive_request(server as Object)
    if m.state<>m.RECV_REQUEST then stop
    ...
    if m.request.isComplete()
        ...
        if m.request.process(m)
            ...
            m.setState(m.DONE)
            m.close = true
            Error("Connection couldn't process request or reply, closing @"+m.client)
        end if
...
function connection_poll_send_header(server as Object) as Boolean
    if m.state<>m.SEND_HEADER then stop
...
function connection_poll_send_reply(server as Object)
    if m.state<>m.SEND_REPLY or m.reply.header_only then stop
