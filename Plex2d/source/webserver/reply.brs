...
function reply_send(sock as Object, bufsize as Integer) as Integer
    ...
    if m.source=m.FROMFILE
        if m.start+m.sent>=m.buf_start+m.buf.count()
            ...
            Info("Read" + Stri(m.buf.count()) + " bytes from source file @" + tostr(m.buf_start))
        end if
    ...
    if buf_remaining=0 then Info("Reply source buf is empty")
    ...
    for each item in dirList
        if item.left(1)<>"."
            ...
            if indicator<>invalid
                ...
                Warn("Reply insufficient stat info to include '" + dir + item + "'")
            end if
        end if
    end for
...
function reply_process_get() as Boolean
    ...
    if stat.type="directory"
        ...
        Info("Reply uri="+uri+", target="+path+", content-type="+m.mimetype+", content-length="+Stri(m.fileLength).trim())
    else
    ...
    if if_mod_since<>invalid
        ...
        if reqDate<>invalid and reqDate.toSeconds() >= m.lastMod.toSeconds()
            ...
            Info("Reply not modified since " + lastMod)
    ...
    if req.range_begin_given or req.range_end_given
        ...
        Info("Reply sending range " + makeRange(start,m.length,m.fileLength))
    else ' no range stuff
...
function reply_log(recent as Integer,from as Integer, total as Integer)
    Info("Sent" + Stri(recent) + " [" + makeRange(from,recent,total) +"]" )
end function
