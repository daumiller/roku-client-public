...
sub usercardInitComponents()
    ...
    if m.user.thumb <> invalid then
        thumb = iif(instr(1, m.user.thumb, "gravatar") > 0, m.user.thumb + "&s=8", m.user.thumb)
        m.bkg = createImage(thumb, m.height, m.height)
    ...
    ' thumb image
    m.thumbBorder = createBlock(&hfffffff20)
    m.AddComponent(m.thumbBorder)
...
sub usercardPerformLayout()
    ...
    m.thumbBorder.SetFrame(xOffset - border, yOffset - border, m.thumb.width + (border * 2), m.thumb.height + (border * 2))
